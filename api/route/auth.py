...
from api.utils.encryption import decrypt_message, encrypt_message
...
def register():
    ...
    if not checkuser:
        ...
        if user:
            ...
            return jsonify(status="error", message="Email not sended"), HTTP_400_BAD_REQUEST
...
def verifyCode():
    ...
    if userValidation:
        return {"message": "User already verified", "alert": "success"}, HTTP_200_OK
    if count5min > 5:
