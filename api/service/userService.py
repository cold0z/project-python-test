...
def insert_user(user=User):
    try:
        conn = get_db_connection()
        cur = conn.cursor()
        cur.execute("INSERT INTO USERS (email,password,registration_date, active, code, code_date) values (%s,%s,%s,%s,%s,%s)  ", [
                    user.email, user.password, user.registration_date, user.active, user.code, user.code_date])
        conn.commit()
        cur.close()
        conn.close()
        return True
...
def verify_auth(email, password):
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT * FROM users WHERE email = TRIM(%s) and password = TRIM(%s)", [email, password])
    data = cur.fetchone()
    cur.close()
    conn.close()
    return data
